...
Where, ${n = 0 \ldots N-1, N}$ is the length of signal, ${x(n)}$ is the function of the signal. Above the expression, with a small extention:

$${X_k+_N = \sum_{n=0}^{N-1} x_n \cdot e^{\frac{-2i{\pi}\left({k+N}\right)n}{N}} = \sum_{n=0}^{N-1} x_n \cdot e^{\frac{-2i{\pi}kn}{N}} \cdot e^{-2i{\pi}n}}$$

note that ${e^{-2i{\pi}n}=1}$, therefore, we have :

$$ {X_k+_N = \sum_{n=0}^{N-1} x_n \cdot e^{\frac{-2i{\pi}\left({k+N}\right)n}{N}} = X_k} $$

so that for any integer ${i}$:

$$X _k+_i \cdot _N = X{_k}_i $$
