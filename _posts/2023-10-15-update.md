...
$${X_{k+N} = \sum_{n=0}^{N-1} x_n \cdot e^{\frac{-2i{\pi}\left({k+N}\right)n}{N}}}$$

$${= \sum_{n=0}^{N-1} x_n \cdot e^{\frac{-2i{\pi}kn}{N}} \cdot e^{-2i{\pi}n}}$$
```python
note that ${e^{-2i{\pi}n}=1}$, therefore, we have :
```
$$ {X_{k+N} = \sum_{n=0}^{N-1} x_n \cdot e^{\frac{-2i{\pi}\left({k+N}\right)n}{N}} = X_k} $$

so that for any integer ${i}$:

$$X _{k+i} \cdot _N = X{_k}_i $$
